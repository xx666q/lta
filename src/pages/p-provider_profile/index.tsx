...
const ProviderProfilePage: React.FC = () => {
  ...
  const [providerProfile, setProviderProfile] = useState<ProviderProfile>({
    ...
    avatar: 'https://s.coze.cn/image/Ry75W4KQT5M/',
    rating: 4.8,
    completedOrders: 125,
    serviceTypes: ['日托服务', '周托服务', '临时照看']
  });
  const [editProfileForm, setEditProfileForm] = useState({
    nickname: providerProfile.nickname,
    phone: '13912347777',
  ...
  return (
    <div className={styles.pageWrapper}>
        ...
        <div className="flex items-center justify-between h-full px-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              ...
              >
                <img src="https://s.coze.cn/image/soyb6vuQbfA/" 
                     alt="服务商头像" className="w-8 h-8 rounded-full" />
      ...
      <div className="flex pt-16">
        {/* 左侧菜单 */}
        <aside className={`${styles.glassSidebar} w-64 min-h-screen p-4`} data-testid="sidebar">
          <nav className="space-y-2">
            <Link to="/provider-dashboard" className={`${styles.menuItem} flex items-center space-x-3 px-4 py-3 rounded-lg text-text-secondary hover:text-secondary transition-colors`}>
              <i className="fas fa-home text-lg"></i>
              <span className="font-medium">工作台</span>
            </Link>
            <Link to="/qualification-audit" className={`${styles.menuItem} flex items-center space-x-3 px-4 py-3 rounded-lg text-text-secondary hover:text-secondary transition-colors`}>
              <i className="fas fa-certificate text-lg"></i>
              <span className="font-medium">资质审核</span>
            </Link>
            <Link to="/service-publish" className={`${styles.menuItem} flex items-center space-x-3 px-4 py-3 rounded-lg text-text-secondary hover:text-secondary transition-colors`}>
            ...
            <Link to="/withdrawal" className={`${styles.menuItem} flex items-center space-x-3 px-4 py-3 rounded-lg text-text-secondary hover:text-secondary transition-colors`}>
            ...
            <Link to="/provider-profile" className={`${styles.menuItem} ${styles.menuItemActive} flex items-center space-x-3 px-4 py-3 rounded-lg text-text-primary`} data-testid="profile-link">
        ...
        <main className="flex-1 p-6 min-h-screen">
          ...
          <header className="mb-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-accent mb-2">服务商个人中心</h2>
              ...
              <div className="flex items-start space-x-6">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-semibold text-accent mb-1">{providerProfile.nickname}</h3>
                      <p className="text-text-secondary">{providerProfile.phone}</p>
                    </div>
                    <button 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-text-secondary">{providerProfile.address}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-user-tag text-secondary w-5"></i>
                      <span className="text-text-secondary">{providerProfile.identity}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-calendar text-secondary w-5"></i>
                      <span className="text-text-secondary">{'注册时间：' + providerProfile.registerTime}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-star text-secondary w-5"></i>
                      <span className="text-text-secondary">{'综合评分：' + providerProfile.rating}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-check-circle text-secondary w-5"></i>
                      <span className="text-text-secondary">{'已完成订单：' + providerProfile.completedOrders}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-list-ul text-secondary w-5"></i>
                      <span className="text-text-secondary">{'服务类型：' + providerProfile.serviceTypes.join('、')}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {/* 服务商功能区 */}
          <section className="mb-8">
            <h3 className="text-lg font-semibold text-accent mb-4">服务商功能</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* 资质审核入口 */}
              <div className={`${styles.glassCard} p-6 rounded-xl cursor-pointer hover:bg-white/30 transition-colors`} onClick={() => navigate('/qualification-audit')}>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-secondary/20 rounded-lg flex items-center justify-center">
                    <i className="fas fa-certificate text-secondary text-xl"></i>
                  </div>
                  <div>
                    <h4 className="font-semibold text-accent">资质审核</h4>
                    <p className="text-text-light text-sm">提交和管理资质材料</p>
                  </div>
                </div>
              </div>
              
              {/* 服务发布入口 */}
              <div className={`${styles.glassCard} p-6 rounded-xl cursor-pointer hover:bg-white/30 transition-colors`} onClick={() => navigate('/service-publish')}>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-secondary/20 rounded-lg flex items-center justify-center">
                    <i className="fas fa-plus-circle text-secondary text-xl"></i>
                  </div>
                  <div>
                    <h4 className="font-semibold text-accent">服务发布</h4>
                    <p className="text-text-light text-sm">创建和管理托管服务</p>
                  </div>
                </div>
              </div>
              
              {/* 收入提现入口 */}
              <div className={`${styles.glassCard} p-6 rounded-xl cursor-pointer hover:bg-white/30 transition-colors`} onClick={() => navigate('/withdrawal')}>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-secondary/20 rounded-lg flex items-center justify-center">
                    <i className="fas fa-money-bill-wave text-secondary text-xl"></i>
                  </div>
                  <div>
                    <h4 className="font-semibold text-accent">收入提现</h4>
                    <p className="text-text-light text-sm">查看收入和申请提现</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section className="mb-8">
            ...
            <div className={`${styles.glassCard} rounded-xl overflow-hidden`}>
              {Object.entries(notificationSettings).map(([key, value], index) => {
                const labels = {
                  orderNotification: { title: '订单通知', desc: '接收新订单和订单状态变更通知' },
                  ownerMessage: { title: '主人消息', desc: '接收宠物主人的消息通知' },
                  systemNotification: { title: '系统通知', desc: '接收平台系统通知和公告' },
                  marketingNotification: { title: '营销活动', desc: '接收优惠活动和新功能通知' }
                };
                
                const iconMap = {
                  orderNotification: 'fas fa-bell',
                  ownerMessage: 'fas fa-comment',
                  systemNotification: 'fas fa-bullhorn',
